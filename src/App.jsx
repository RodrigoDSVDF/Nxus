...
const bentoFeatures = [
    ...
    {
        colSpan: "md:col-span-2",
        bgImage: brainAIImg, // Alterado para dar mais variedade
        ...
        <header className="sticky top-0 z-50 w-full bg-[#0A0A0A]/90 backdrop-blur-md border-b border-[#333] shadow-lg">
            <div className="max-w-7xl mx-auto h-20 flex items-center justify-between px-4">
                ...
                <button onClick={() => handleScroll('hero')} className="flex items-center gap-2 cursor-pointer">
                    <Brain className="w-7 h-7 text-[--color-nexus-teal]" />
                ...
                <nav className="hidden lg:flex items-center gap-6">
                    {NAV_ITEMS.map((item) => (
                        <button
                            ...
                            className="flex items-center text-sm font-medium text-gray-400 hover:text-[--color-nexus-teal] transition-colors group"
                        >
                            <item.icon className="w-4 h-4 mr-1 text-[--color-nexus-teal] group-hover:text-white transition-colors" />
                            {item.title}
                        </button>
                    ))}
                </nav>
                ...
                <a href={CHECKOUT_LINK} target="_blank" rel="noopener noreferrer">
                    <Button className="h-10 px-6 text-sm font-semibold bg-[--color-nexus-orange] hover:bg-red-600 transition-colors">
                        ACESSO IMEDIATO
                    </Button>
                </a>
            </div>
...
function App() {
    ...
    // NOVO ESTADO: Qual feature do Bento Grid está selecionada para destaque
    const [activeFeature, setActiveFeature] = useState(bentoFeatures[0]);

    const MID_BANNER_COLOR = '#4E97D1';
    const MID_BANNER_TEXT_COLOR = '#DBEEFF';
    ...
    return (
        <div className="min-h-screen bg-[--color-nexus-bg] font-['Poppins',sans-serif] overflow-x-hidden text-slate-100 selection:bg-[--color-nexus-teal] selection:text-black">
            ...
            {/* 5. A SOLUÇÃO: BENTO GRID - AGORA COM MENU INTERATIVO */}
            <section id="bento" ref={bentoRef} className="relative z-10 pt-32 pb-16 px-4 bg-[#0A0A0A]">
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className={`transition-all duration-1000 delay-300 ${bentoVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
                        
                        {/* DESTAQUE PRINCIPAL (IMAGEM E DESCRIÇÃO DO ITEM ATIVO) */}
                        <div className="nexus-card bg-[#1A2A3A] p-8 md:p-12 rounded-3xl mb-10 border border-[--color-nexus-teal]/30 flex flex-col md:flex-row gap-8 items-center">
                            <div className="w-full md:w-1/2 relative h-64 md:h-96 rounded-2xl overflow-hidden shadow-xl">
                                {/* IMAGEM DE DESTAQUE */}
                                {activeFeature.bgImage ? (
                                    <img 
                                        src={activeFeature.bgImage} 
                                        alt={`Destaque: ${activeFeature.title}`} 
                                        className="w-full h-full object-cover transition-transform duration-500 hover:scale-105"
                                    />
                                ) : (
                                    <div className="w-full h-full flex items-center justify-center bg-[#111] text-3xl font-bold text-[--color-nexus-teal]">
                                        <activeFeature.icon className="w-10 h-10 mr-3"/>
                                        {activeFeature.title}
                                    </div>
                                )}
                            ...
                            <div className="w-full md:w-1/2 space-y-4">
                                <activeFeature.icon className="w-10 h-10 text-[--color-nexus-orange]"/>
                                <h3 className="text-3xl font-extrabold text-white">{activeFeature.title}</h3>
                                <p className="text-xl text-gray-300 leading-relaxed">{activeFeature.desc}</p>
                                <a href={CHECKOUT_LINK} target="_blank" rel="noopener noreferrer">
                                    <Button 
                                        className="mt-4 bg-[--color-nexus-orange] hover:bg-red-600 transition-colors"
                                    >
                                        Quero Esta Vantagem <ArrowRight className="w-4 h-4 ml-2"/>
                                    </Button>
                                </a>
                            </div>
                        </div>
                        ...
                        {/* MENU INTERATIVO: BOTÕES DE SELEÇÃO */}
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            {bentoFeatures.map((item, index) => (
                                <button 
                                    ...
                                    className={`
                                        nexus-card p-6 rounded-2xl text-left transition-all duration-300
                                        ${activeFeature.title === item.title 
                                            ? 'bg-[--color-nexus-teal]/10 border-2 border-[--color-nexus-teal] shadow-lg scale-[1.02]' 
                                            : 'bg-[#111]/50 border border-[#333] hover:border-[--color-nexus-teal]/50'
                                        }
                                    `}
                                >
                                    <div className="flex items-center mb-3">
                                        <item.icon className={`w-6 h-6 mr-3 ${activeFeature.title === item.title ? 'text-[--color-nexus-teal]' : 'text-gray-400'}`} />
                                        <h3 className="text-lg font-bold text-white group-hover:text-[--color-nexus-teal]">{item.title}</h3>
                                    </div>
