...
const testimonials = [
  { img:socialMan,    name:"Rafael C.",  role:"Consultor de Estratégia",  text:"O Nexus não é mais um curso, é um ecossistema. O hub de ferramentas atualizadas melhorou minha produtividade — e o e-book transformou meu processo de tomada de decisão." },
  { img:socialWoman1, name:"Camila L.",  role:"Head de Inovação",          text:"Finalmente entendi que produtividade não é sobre fazer mais, mas sobre pensar melhor. O framework do Nexus mudou minha abordagem por completo." },
  { img:socialWoman2, name:"Sandra M.",  role:"Empreendedora Digital",     text:"Do caos ao controle. O plano de implementação é o diferencial que faltava — não preciso mais gastar horas procurando as ferramentas certas." },
]
...
const faqItems = [
  ...
  { q:"Como funciona a garantia de 7 dias?",      a:"Teste o sistema por 7 dias. Se não sentir melhora tangível na sua clareza e produtividade, um e-mail devolve 100% do investimento." },
]
    ...
    <div style={{ minHeight:'100vh', background:C.ink, color:C.ice, fontFamily:'Inter, sans-serif', overflowX:'hidden' }}>
      ...
      <section id="video" ref={videoRef} style={{ position:'relative', zIndex:10, padding:'80px 1.5rem', background:C.navy }}>
        <div style={{ ...trans(videoV), maxWidth:'960px', margin:'0 auto' }}>
          ...
          <div style={{ position:'relative', borderRadius:'12px', overflow:'hidden', border:`1px solid ${C.steel}`, boxShadow:`0 32px 64px rgba(0,0,0,0.4)` }}>
            ...
            <div style={{ padding:'20px 24px', background:C.navy, borderTop:`1px solid ${C.steel}`, display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'12px' }}>
              <div>
                ...
                <div style={{ color:C.silver, fontSize:'0.78rem', marginTop:'2px' }}>Duração: 2:15 · Publicado: Outubro 2025</div>
              </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'12px', marginTop:'32px' }}>
            {[['15min','Setup Completo'],['72h','Primeiros Resultados'],['10x','Produtividade'],['100%','Satisfação']].map(([v,l],i)=>(
              <div key={i} style={{ textAlign:'center', padding:'20px 12px', background:C.ink, border:`1px solid ${C.steel}`, borderRadius:'10px' }}>
                <div style={{ fontFamily:'Space Grotesk, sans-serif', fontSize:'1.6rem', fontWeight:800, color:C.sapphire2, lineHeight:1 }}>{v}</div>
                <div style={{ color:C.silver, fontSize:'0.75rem', marginTop:'6px', letterSpacing:'0.04em' }}>{l}</div>
              </div>
            ))}
          </div>
      ...
      <div style={{ borderTop:`1px solid ${C.steel}`, borderBottom:`1px solid ${C.steel}`, padding:'28px 1.5rem', background:C.ink }}>
        <div style={{ maxWidth:'1000px', margin:'0 auto', textAlign:'center' }}>
          <div className="eyebrow" style={{ marginBottom:'20px', color:C.silver }}>Método utilizado por profissionais de</div>
          <div style={{ display:'flex', flexWrap:'wrap', justifyContent:'center', gap:'40px', opacity:0.4, filter:'grayscale(1)' }}>
            {['TECH CORP','InnovateLabs','Future / Work','Global Systems'].map((n,i)=>(
              <span key={i} style={{ fontSize:'1.1rem', fontWeight:700, color:C.ice, fontFamily:'Space Grotesk, sans-serif' }}>{n}</span>
            ))}
          </div>
        </div>
      </div>
        ...
        <div style={{ ...trans(stackV), maxWidth:'820px', margin:'0 auto' }}>
          ...
          <div style={{ marginTop:'48px', textAlign:'center' }}>
            ...
            <div style={{ marginBottom:'28px' }}>
              ...
              <div className="eyebrow" style={{ marginTop:'10px', color:C.sapphire2 }}>Oferta de Lançamento Exclusiva</div>
            </div>
            ...
            <a href={CHECKOUT_LINK} target="_blank" rel="noopener noreferrer" onClick={trackCheckout}
              ...
              onMouseLeave={e=>e.currentTarget.style.background=C.sapphire}
            >
              SIM, QUERO ACESSO VITALÍCIO <ArrowRight size={18}/>
            </a>
      ...
      <section id="reviews" ref={reviewsRef} style={{ padding:'80px 1.5rem', background:C.navy, borderTop:`1px solid ${C.steel}`, borderBottom:`1px solid ${C.steel}` }}>
        <div style={{ ...trans(reviewsV), maxWidth:'1100px', margin:'0 auto' }}>
          <div style={{ textAlign:'center', marginBottom:'52px' }}>
            <div className="eyebrow" style={{ marginBottom:'12px' }}>Depoimentos</div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'14px' }} className="grid-cols-1 md:grid-cols-3">
            {testimonials.map((item,i)=>(
              <div key={i} className="nexus-card" style={{ padding:'28px 24px', display:'flex', flexDirection:'column', justifyContent:'space-between' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:'12px', paddingTop:'16px', borderTop:`1px solid ${C.steel}` }}>
                  <div style={{ width:'40px', height:'40px', borderRadius:'50%', overflow:'hidden', border:`2px solid ${C.steel}`, flexShrink:0 }}>
                    <img src={item.img} alt={item.name} style={{ width:'100%', height:'100%', objectFit:'cover' }}/>
                  </div>
                  <div>
                    <div style={{ fontWeight:600, color:C.ice, fontSize:'0.88rem' }}>{item.name}</div>
                    <div style={{ color:C.silver, fontSize:'0.75rem', marginTop:'1px' }}>{item.role}</div>
                  </div>
                </div>
              </div>
            ))}
