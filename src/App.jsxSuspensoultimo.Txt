...
import {
  ...
  Zap as ZapIcon, BarChart3, BookOpen, X
} from 'lucide-react'
import './App.css'
...
const faqItems = [
  ...
  { 
    ...
    a: "Sim. O Nexus foi desenhado para ser um sistema de orientação. O objetivo é guiar você desde os fundamentos básicos até formas mais avançadas de uso da IA." 
  },
  { 
  ...
  { 
    ...
    a: "Simples: Teste o sistema por 7 dias. Se você não sentir uma melhora tangível na sua clareza mental e produtividade, basta um e-mail que devolvemos 100% do seu dinheiro." 
  },
];
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3 rounded-t-2xl"
          style={{ background: '#0f0f0f', border: '1px solid #333', borderBottom: 'none' }}>
          <div className="flex items-center gap-3">
            <BookOpen className="w-5 h-5 text-[#2DD4BF]" />
            <span className="text-white font-bold text-sm tracking-wide">Manual Prático de IA no Dia a Dia</span>
          </div>
          <div className="flex items-center gap-3">
            <a
              href={CHECKOUT_LINK}
              ...
              className="hidden sm:inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold text-white"
              style={{ background: 'linear-gradient(90deg, #FF6B35, #FF4F1F)', border: '1px solid #fbbf24' }}
            >
              QUERO O NEXUS COMPLETO →
            </a>
            <button
              onClick={onClose}
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-[#0A0A0A]/90 backdrop-blur-md border-b border-[#333] shadow-lg">
      <div className="max-w-7xl mx-auto h-20 flex items-center justify-between px-4">
        <button onClick={() => handleScroll('hero')} className="cursor-pointer group">
          <div className="relative">
            <img 
              src={logoImg} 
              alt="Nexus Manual" 
              className="max-w-[180px] md:max-w-[220px] lg:max-w-[260px] w-auto h-auto object-contain group-hover:scale-105 transition-transform duration-300"
            />
            <div className="absolute inset-0 bg-[#2DD4BF] rounded-full blur-md opacity-0 group-hover:opacity-30 transition-opacity"></div>
          </div>
        </button>
        ...
        <nav className="hidden lg:flex items-center gap-6">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-sm font-medium text-[#2DD4BF] hover:text-white border border-[#2DD4BF]/40 hover:border-[#2DD4BF] px-3 py-1.5 rounded-full transition-all"
          >
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] font-['Poppins',sans-serif] overflow-x-hidden text-slate-100 selection:bg-[#FF6B35] selection:text-white">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="relative w-full max-w-[1800px] h-full">
            <img
              ...
              style={{ filter: 'contrast(1.1) brightness(1.1) saturate(1.2)', mixBlendMode: 'normal' }}
            />
          </div>
        </div>
      ...
      <section id="hero" ref={heroRef} className="relative z-10 pt-20 pb-32 px-4 overflow-hidden">
        <div className={`max-w-7xl mx-auto text-center transition-all duration-1000 ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <div className="flex flex-col items-center gap-4 animate-scale-in">
            ...
            {/* Botão Manual Grátis no Hero */}
            <button
              onClick={() => setShowManual(true)}
              className="group flex items-center gap-2 px-6 py-3 rounded-xl border border-[#2DD4BF]/30 hover:border-[#2DD4BF] text-[#2DD4BF] hover:text-white text-sm font-semibold transition-all duration-300 hover:bg-[#2DD4BF]/10"
            >
              <BookOpen className="w-4 h-4 group-hover:scale-110 transition-transform" />
              📘 Ver Manual Gratuito de IA no Dia a Dia
            </button>
      ...
      <section id="video" ref={videoRef} className="relative z-10 py-20 px-4">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${videoVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'}`}>
            ...
            <div className="relative bg-gradient-to-br from-[#0A0A0A] to-[#1A1A1A] rounded-2xl overflow-hidden border-2 border-gray-800 group-hover:border-[#2DD4BF]/50 transition-all duration-500 shadow-2xl">
              <div className="relative aspect-video">
                ...
                <iframe
                  ...
                  title="Demonstração do Sistema Nexus"
                  frameBorder="0"
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mt-12">
            {[
              { val: "15min", label: "Setup Completo" },
              { val: "72h", label: "Primeiros Resultados" },
              { val: "10x", label: "Produtividade" },
              { val: "100%", label: "Satisfação Garantida" },
            ].map((s, i) => (
              <div key={i} className="text-center p-4 md:p-6 bg-[#111]/50 rounded-xl border border-gray-800 hover:border-[#2DD4BF]/30 transition-colors group">
                <div className="text-2xl md:text-3xl font-bold text-[#2DD4BF] mb-2 group-hover:scale-110 transition-transform">{s.val}</div>
          ...
          <div className="text-center mt-12">
            <a
              ...
              rel="noopener noreferrer"
              onClick={handleTrackCheckout}
              className="group inline-block w-full max-w-lg mx-auto"
            >
              <Button className="relative overflow-hidden bg-gradient-to-r from-[#FF6B35] to-[#FF4F1F] hover:from-[#FF4F1F] hover:to-[#FF6B35] text-white font-bold py-5 px-8 rounded-xl text-lg md:text-xl transition-all hover:scale-105 cursor-pointer shadow-2xl shadow-[#FF6B35]/30 w-full border-2 border-yellow-400 hover:border-yellow-300">
                <span className="relative z-10 flex items-center justify-center gap-3 whitespace-normal text-center leading-tight">
        ...
        <div className={`max-w-5xl mx-auto transition-all duration-1000 ${aboutVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'}`}>
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-gray-400 max-w-4xl mx-auto mb-12">
              O Nexus atua sobre um dos principais desafios da era digital: <span className="text-[#FF6B35] font-semibold">manter-se atualizado diante da rápida evolução das tecnologias emergentes</span>, servindo como um guia confiável nesse processo.
            </p>
          </div>
      ...
      <section id="bento" ref={bentoRef} className="relative z-10 pt-20 md:pt-32 pb-16 px-4 bg-[#0A0A0A]">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mt-10 md:mt-16 px-4">
            <a href={CHECKOUT_LINK} target="_blank" rel="noopener noreferrer" onClick={handleTrackCheckout} className="group inline-block w-full max-w-lg mx-auto">
              <Button className="relative overflow-hidden w-full bg-gradient-to-r from-[#FF6B35] to-[#FF4F1F] hover:from-[#FF4F1F] hover:to-[#FF6B35] text-white font-bold py-5 px-8 rounded-xl text-lg md:text-xl transition-all hover:scale-105 cursor-pointer shadow-2xl shadow-[#FF6B35]/30 border-2 border-yellow-400 hover:border-yellow-300">
                <span className="relative z-10 flex items-center justify-center gap-3 whitespace-normal text-center leading-tight">
      ...
      <div className="relative z-20 py-6 overflow-hidden bg-[#0A0A0A] border-y border-[#333] horizontal-banner-mid-section">
        <div className="horizontal-banner-wrapper" style={{ background: `linear-gradient(90deg, ${TEAL} 0%, ${BG} 50%, ${ORANGE} 100%)`, border: 'none', opacity: 0.8 }}>
          <div className="horizontal-banner">
            <div className="banner-content" style={{ animationDuration: '10s' }}>
              {['BIOHACKING DO FOCO', 'METODOLOGIAS ATIVAS', 'DEEP WORK', 'INTELIGÊNCIA ESTRATÉGICA'].map((t, i) => (
                <span key={i} className="banner-item font-mono tracking-widest flex items-center text-base md:text-lg text-white">
                  <BrainIcon className="w-4 h-4 md:w-5 md:h-5 mr-2 md:mr-3 text-[#FF6B35]" /> {t}
                </span>
              ))}
              {['BIOHACKING DO FOCO', 'METODOLOGIAS ATIVAS', 'DEEP WORK', 'INTELIGÊNCIA ESTRATÉGICA'].map((t, i) => (
                <span key={`r${i}`} className="banner-item font-mono tracking-widest flex items-center text-base md:text-lg text-white">
                  <BrainIcon className="w-4 h-4 md:w-5 md:h-5 mr-2 md:mr-3 text-[#FF6B35]" /> {t}
                </span>
              ))}
            </div>
        ...
        <div className="max-w-5xl mx-auto bg-[#1A2A3A]/80 backdrop-blur-xl border border-[#333] rounded-2xl md:rounded-[2.5rem] p-6 md:p-16 relative overflow-hidden">
          ...
          <div className={`relative z-10 transition-all duration-1000 ${stackVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'}`}>
            <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-center mb-8 md:mb-12">
              <span className="animated-gradient-title">O QUE VOCÊ VAI RECEBER EXATAMENTE</span>
            </h2>
            <div className="mb-6 md:mb-8 text-center">
              <div className="inline-block px-4 py-2 md:px-6 md:py-3 bg-gradient-to-r from-[#2DD4BF]/20 to-[#FF6B35]/20 rounded-xl group hover:scale-105 transition-transform duration-300 glow-border">
                <p className="text-lg md:text-2xl font-bold shimmer-title">PACOTE NEXUS COMPLETO: ACESSO VITALÍCIO</p>
              </div>
            </div>
            ...
            <div className="mt-12 md:mt-16 text-center">
              ...
              <a href={CHECKOUT_LINK} target="_blank" rel="noopener noreferrer" className="inline-block w-full max-w-md mx-auto" onClick={handleTrackCheckout}>
      ...
      <section id="reviews" ref={reviewsRef} className="relative z-10 py-16 md:py-24 px-4 bg-[#0A0A0A] border-y border-[#333]">
        <div className={`max-w-6xl mx-auto transition-all duration-1000 ${reviewsVisible ? 'opacity-100' : 'opacity-0'}`}>
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-center mb-12 md:mb-16">
            <span className="dynamic-gradient">Quem já Implementou o Sistema Nexus</span>
          </h2>
      ...
      <section id="faq" className="relative z-10 py-16 md:py-24 px-4">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-center mb-8 md:mb-12">
            <span className="animated-gradient-title">Perguntas Frequentes (FAQ)</span>
          </h2>
        ...
        <div className={`max-w-4xl mx-auto text-center relative z-20 transition-all duration-1000 ${offerVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
          <div className="mb-8 md:mb-12">
            ...
            <h2 className="text-2xl md:text-3xl lg:text-5xl font-bold mb-4 md:mb-6">
              <span className="dynamic-gradient glow-text">Garantia de 7 Dias: Zero Risco</span>
            </h2>
            <p className="text-lg md:text-xl text-gray-300 max-w-2xl mx-auto mb-6 md:mb-8 leading-relaxed px-4">
              <span className="text-[#2DD4BF] font-semibold">Simples: Teste o sistema por 7 dias.</span> Se você não sentir uma melhora tangível na sua clareza mental e produtividade, basta um e-mail que devolvemos 100% do seu dinheiro.
            </p>
          ...
          <div className="bg-[#1A2A3A] border-2 border-[#FF4F1F] rounded-2xl md:rounded-[3rem] p-6 md:p-12 shadow-2xl shadow-[#FF4F1F]/20 relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <h3 className="text-xl md:text-2xl lg:text-3xl font-bold text-white mb-2">
                <span className="shimmer-title">Última Chance para Garantir o Preço de Lançamento</span>
              </h3>
                  ...
                  <div className="flex flex-col gap-1 md:gap-2">
                    {["E-book Digital + Vídeo-Aulas", "Blueprint de Implementação (7 Dias)", "Banco de Prompts Estratégicos", "Atualizações Vitalícias Inclusas"].map((t, i) => (
                      <div key={i} className="flex items-center text-xs md:text-sm text-gray-300">
                        <Check className="w-3 h-3 md:w-4 md:h-4 text-[#2DD4BF] mr-2" /> {t}
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <a href={CHECKOUT_LINK} target="_blank" rel="noopener noreferrer" className="block max-w-2xl mx-auto" onClick={handleTrackCheckout}>
                <Button className="relative overflow-hidden pulse-button w-full h-auto py-5 text-white font-extrabold text-lg md:text-xl rounded-xl md:rounded-2xl flex items-center justify-center gap-4 hover:scale-[1.02] transition-transform whitespace-normal text-center border-2 border-yellow-400 hover:border-yellow-300">
                  <span className="relative z-10">ACESSAR MEU SISTEMA NEXUS AGORA →</span>
                </Button>
              </a>
            </div>
          </div>
      ...
      {/* BOTÃO FLUTUANTE MANUAL — mobile e desktop */}
      <button
        onClick={() => setShowManual(true)}
        className="fixed bottom-6 left-6 z-40 flex items-center gap-2 px-4 py-3 rounded-full shadow-2xl font-bold text-sm transition-all hover:scale-105 active:scale-95"
        style={{
          background: 'linear-gradient(135deg, #0f0f0f 0%, #1A2A3A 100%)',
          border: '1px solid #2DD4BF',
          color: '#2DD4BF',
          boxShadow: '0 0 20px rgba(45,212,191,0.2)'
        }}
        title="Abrir Manual Gratuito de IA"
      >
        <BookOpen className="w-4 h-4 flex-shrink-0" />
        <span className="hidden sm:inline">Manual Grátis de IA</span>
      </button>
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-4 gap-8 md:gap-12 mb-12 md:mb-16">
          ...
          <div>
            ...
            <ul className="space-y-2 md:space-y-3 text-gray-500 text-sm md:text-base">
              ...
              <li>
                <button onClick={() => setShowManual(true)} className="hover:text-[#2DD4BF] transition-colors flex items-center gap-1.5">
                  <BookOpen className="w-3.5 h-3.5" /> Manual Gratuito de IA
                </button>
              </li>
